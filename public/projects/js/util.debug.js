...
var isNaN = Number.isNaN || window.isNaN || function (n) {
...
function linkCheckboxToBoolean(checkbox, object, attr, func) {
    ...
    checkbox.addEventListener('click', function () {
        object[attr] = checkbox.checked;func();
    });
}
...
function linkInputToNumber(input, object, attr, func, instant) {
    ...
    if (instant) input.addEventListener('input', function () {
        if (!isNaN(input.valueAsNumber)) object[attr] = input.valueAsNumber;func();
...
QuadTree.prototype.insert = function (obj) {
    ...
    return this.nw.insert(obj) || this.ne.insert(obj) || this.sw.insert(obj) || this.se.insert(obj);
};
...
function poissonDisk(minx, miny, width, height, r) {
    ...
    var points = [{ x: Math.random() * width + minx, y: Math.random() * height + miny }];
    ...
    for (var i = 0; i < points.length; i++) {
        for (var j = 0; j < points.length; j++) {
            ctx.fillRect(cvs.drawWidth / 2 + (points[j].x - points[i].x), cvs.drawHeight / 2 + (points[j].y - points[i].y), 1, 1);
        }
    }
