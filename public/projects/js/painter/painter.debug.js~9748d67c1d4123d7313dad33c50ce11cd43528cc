...
window.onload = function () {
    ...
    function init() {
        ...
        inputs.neighborsZoomInBtn.addEventListener('click', function () {
            neighborData.scale++;drawNeighbors();
        });
        ...
        switch (options.pattern) {
            ...
            case 'random':
                for (i = 0; i < options.numPoints; i++) {
                    cell = Cell(Math.floor(Math.random() * width), Math.floor(Math.random() * height), options.baseRGB);
                    deviate(cell, options.initialDeviation);
                    add(cell);
                }
                break;
    ...
    function hideOptionals() {
        ...
        i = optionals.length;
        while (i--) {
            optionals[i].style.display = 'table-row';
        }
    }
    ...
    function drawNeighbors() {
        ...
        while (i--) {
            ...
            neighborContext.fillRect(skip * (n.x + neighborData.offsetX), skip * (n.y + neighborData.offsetY), neighborData.scale - neighborData.padding, neighborData.scale - neighborData.padding);
        }
    }
