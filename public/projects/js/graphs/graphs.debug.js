...
window.onload = function () {
 ...
 var image = 0;*/
	...
	var updates = {
		...
		setAll: function setAll(b) {
			this.mouseMotion = this.mousePress = this.mouseRelease = this.mouseEnter = this.mouseLeave = this.nodesChanged = this.edgesChanged = this.orderChanged = this.selection = this.deselection = this.graph = this.background = b;
		}
	};
  ...
  options.range = 2500;
		...
		loadFiles(['shader.vert', 'shader.frag'], function (files) {
			...
			for (var i = 0; i < 16; i++) {
				// MAX_EDGES is 16 in shader.frag
	...
	function draw() {
		...
		var bgchanged = options.background && (updates.background || (updates.edgesChanged || updates.deselection) && (options.constantUpdates || !updates.mouseMotion && !updates.mousePress) || updates.orderChanged);
		...
		var distschanged = options.edgeDists && (updates.mouseMotion || updates.mouseEnter || updates.mouseLeave);
	...
	function drawBackground() {
		...
		for (var i = 0; i < edges.length; i++) {
			var edge = edges[i];
			gl.uniform4fv(uniforms.edges[i], new Float32Array([edge.a.x, drawCanvas.drawHeight - edge.a.y, edge.b.x, drawCanvas.drawHeight - edge.b.y]));
		}
	...
	function connectedGraph(width, height, n) {
		...
		for (var i = 0; i < n; i++) {
			var node = new Node(width / 2 + Math.sin(2 * Math.PI * i / n) * (width / 4), height / 2 + Math.cos(2 * Math.PI * i / n) * (height / 4));
			nodes[i] = node;
			for (var j = 0; j < i; j++) {
