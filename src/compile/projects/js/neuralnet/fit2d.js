...
FitGame.prototype.run = function(net) {
    var fit = function(n, x, y, f, a) {n.input(x, y); n.fitness += f(n.output(0), a);};
    ...
    // fit(net, 0.0, 0.0, f, (0 / 6));
    // fit(net, 1.0, 1.0, f, (0 / 6));
    // fit(net, 0.0, 1.0, f, (6 / 6));
    // fit(net, 1.0, 0.0, f, (6 / 6));

    // fit(net, 0.5, 0.5, f, (3 / 6));
    // fit(net, 0.5, 0.0, f, (2 / 6));
    // fit(net, 0.0, 0.5, f, (4 / 6));
    // fit(net, 0.5, 1.0, f, (2 / 6));
    // fit(net, 1.0, 0.5, f, (4 / 6));

    // var f = function(x, d) {return d > 0 ? (x > 0.8 ? (1.0 + (x / 10.0)) : 0.0) * d : (x < 0.2 ? (1.0 + ((1.0 - x) / 10.0)) : 0.0) * -d;};
    // fit(net, 0.1, 0.1, f, -1);
    // fit(net, 0.1, 0.5, f, -1);
    // fit(net, 0.1, 0.9, f, -1);
    // fit(net, 0.5, 0.9, f, -1);
