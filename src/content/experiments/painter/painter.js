window.onload = function() {
  ...
  function drawNeighbors() {
    ...
    var alignY =
      (neighborData.offsetY - Math.ceil(neighborData.offsetY)) * skip;

    for (
      var y = Math.floor(-neighborData.offsetY);
      alignY < neighborData.height;
      y++, alignY += skip
    ) {
      var alignX =
        (neighborData.offsetX - Math.ceil(neighborData.offsetX)) * skip;

      for (
        var x = Math.floor(-neighborData.offsetX);
        alignX < neighborData.width;
        x++, alignX += skip
      ) {
        if (y === 0 && x === 0) {
          neighborContext.fillStyle = '#00FF00';
          neighborContext.strokeRect(
            alignX,
            alignY,
