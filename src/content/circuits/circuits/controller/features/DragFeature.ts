    ...
    switch (e.type) {
      case 'down':
        if (selectedHover && this.controller.selected) {
          this.target = this.controller.selected
            .map((view) => {
              if (view instanceof ConnectionView) return null;
              const { x, y } = View.getRelativePosition(view, canvas);
              return {
                view,
                x: e.root.x - x,
                y: e.root.y - y
        ...
        if (this.target && this.target.length) {
          this.target.forEach((data) => {
            if (data.view === this.controller.canvas) {
              ...
              const { x, y } = data.view.getDimensions();
              data = {
                view: data.view,
                x: data.x - x,
                y: data.y - y
...
function getMoveableTarget(tree: PositionalTree): PositionalTree {
  const found =
    findFirst(tree, (x, l) => l === 1 && !(x.data instanceof ConnectionView)) ||
    tree;
